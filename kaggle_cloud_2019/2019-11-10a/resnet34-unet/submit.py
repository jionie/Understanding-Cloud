...
def run_submit_segmentation(
    ...
    out_dir = \
        '/root/share/project/kaggle/2019/cloud/result/run2/resnet34-unet-fold_a2'
    initial_checkpoint = \
        '/root/share/project/kaggle/2019/cloud/result/run2/resnet34-unet-fold_a2/checkpoint/00014500_model.pth'
    ...
    threshold_mask  = [ 0.30, 0.30, 0.30, 0.30,]
    ...
    if mode =='test':
        ...
        csv_file = out_dir +'/submit/%s/resnet34-unet-th0.70-xxx.csv'%(mode_folder)

        predict_label = probability_label>(np.array(threshold_label)*255).astype(np.uint8).reshape(1,4)
        predict_mask  = probability_mask>(np.array(threshold_mask)*255).astype(np.uint8).reshape(1,4,1,1)
        ...
        encoded_pixel = ['1 1' if r!='' else '' for r in encoded_pixel]
        df = pd.DataFrame(zip(image_id_class_id, encoded_pixel), columns=['Image_Label', 'EncodedPixels'])
        df.to_csv(csv_file.replace('.csv','.probe-yyy.csv'), index=False)
