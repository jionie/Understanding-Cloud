...
def run_test_ensemble_segmentation_only():
    ...
    # model = ['ResNet34-seresnext50-efficientnet-b4-efficientnet-b3', 'deep_se50-deep_se101']
    ...
    for t in range(len(dir)):
        ...
        if t == 0:
            ensemble_label = probability_label * fold[t]
            ensemble_mask  = probability_mask * fold[t]
        else:
            ensemble_label += probability_label * fold[t]
            ensemble_mask  += probability_mask * fold[t]
    ...
    #---
    threshold_label      = [ 0.60, 0.70, 0.55, 0.65,]
    # threshold_label      = [ 0.50, 0.60, 0.45, 0.60,]
